...
router.post("", async (req, res) => {
  try {
    let producer = await Producer.findOne({ Name: req.body.Name })
      .lean()
      .exec();
    if (producer) {
      return res
        .status(400)
        .send({ message: "Producer with that name already exists" });
    }
...
router.patch("/:id", authenticate, async (req, res) => {
  try {
    let producer = await Producer.findByIdAndUpdate(
      req.params.id,
      {
...
router.delete("/:id", authenticate, async (req, res) => {
  try {
    let producer = await Producer.findByIdAndDelete(req.params.id)
      .lean()
      .exec();
    return res.send(producer);
  } catch (err) {
